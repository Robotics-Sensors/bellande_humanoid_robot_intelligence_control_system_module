...
BaseModule::BaseModule()
  ...
  control_mode_ =
      humanoid_robot_intelligence_control_system_framework::PositionControl;

  base_module_state_ = new BaseModuleState();
  joint_state_ = new BaseJointState();
}

BaseModule::~BaseModule() { queue_thread_.join(); }
...
void BaseModule::initialize(
  ...
  for (std::map<std::string,
                humanoid_robot_intelligence_control_system_framework::Dynamixel
                    *>::iterator it = robot->dxls_.begin();
       it != robot->dxls_.end(); it++) {
    std::string joint_name = it->first;
    humanoid_robot_intelligence_control_system_framework::Dynamixel *dxl_info =
        it->second;

    joint_name_to_id_[joint_name] = dxl_info->id_;
    result_[joint_name] =
        new humanoid_robot_intelligence_control_system_framework::
            DynamixelState();
  ...
  /* Load ROS Parameter */
  ros_node.param<std::string>(
      "init_pose_file_path", init_pose_file_path_,
      ros::package::getPath(
  ...
  set_ctrl_module_pub_ = ros_node.advertise<std_msgs::String>(
      "/humanoid_robot_intelligence_control_system/enable_ctrl_module", 1);
}
...
void BaseModule::parseInitPoseData(const std::string &path) {
  ...
  for (YAML::iterator yaml_it = via_pose_node.begin();
    ...
    for (int num = 0; num < via_num; num++)
      base_module_state_->joint_via_pose_.coeffRef(num, id) =
          value[num] * DEGREE2RADIAN;
  }
  ...
  base_module_state_->all_time_steps_ =
      int(base_module_state_->mov_time_ / base_module_state_->smp_time_) + 1;
  base_module_state_->calc_joint_tra_.resize(
      base_module_state_->all_time_steps_, MAX_JOINT_ID + 1);
}
...
void BaseModule::queueThread() {
  ...
  /* subscribe topics */
  ros::Subscriber ini_pose_msg_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/base/ini_pose", 5,
      &BaseModule::initPoseMsgCallback, this);
  set_module_client_ = ros_node.serviceClient<
...
void BaseModule::initPoseMsgCallback(const std_msgs::String::ConstPtr &msg) {
  if (base_module_state_->is_moving_ == false) {
    if (msg->data == "ini_pose") {
      ...
      // wait for changing the module to base_module and getting the goal
      // position
      ...
      tra_gene_tread_ = boost::thread(
          boost::bind(&BaseModule::initPoseTrajGenerateProc, this));
    }
  } else
...
void BaseModule::initPoseTrajGenerateProc() {
  for (int id = 1; id <= MAX_JOINT_ID; id++) {
    ...
    if (base_module_state_->via_num_ == 0) {
      tra = humanoid_robot_intelligence_control_system_framework::
          calcMinimumJerkTra(ini_value, 0.0, 0.0, tar_value, 0.0, 0.0,
      ...
      Eigen::MatrixXd d_via_value =
          base_module_state_->joint_via_dpose_.col(id);
      Eigen::MatrixXd dd_via_value =
          base_module_state_->joint_via_ddpose_.col(id);

      tra = humanoid_robot_intelligence_control_system_framework::
          calcMinimumJerkTraWithViaPoints(
              base_module_state_->via_num_, ini_value, 0.0, 0.0, via_value,
              d_via_value, dd_via_value, tar_value, 0.0, 0.0,
              base_module_state_->smp_time_, base_module_state_->via_time_,
              base_module_state_->mov_time_);
    }

    base_module_state_->calc_joint_tra_.block(
        0, id, base_module_state_->all_time_steps_, 1) = tra;
  }
...
void BaseModule::poseGenerateProc(Eigen::MatrixXd joint_angle_pose) {
  ...
  for (int id = 1; id <= MAX_JOINT_ID; id++) {
    ...
    Eigen::MatrixXd tra = humanoid_robot_intelligence_control_system_framework::
        calcMinimumJerkTra(ini_value, 0.0, 0.0, tar_value, 0.0, 0.0,
                           base_module_state_->smp_time_,
                           base_module_state_->mov_time_);
...
void BaseModule::poseGenerateProc(
...
bool BaseModule::isRunning() { return base_module_state_->is_moving_; }
...
void BaseModule::process(
  ...
  for (std::map<std::string,
                humanoid_robot_intelligence_control_system_framework::
                    DynamixelState *>::iterator state_iter = result_.begin();
    ...
    std::map<std::string,
             humanoid_robot_intelligence_control_system_framework::Dynamixel
                 *>::iterator dxl_it = dxls.find(joint_name);
    if (dxl_it != dxls.end())
      dxl = dxl_it->second;
    else
      continue;
  ...
  if (base_module_state_->is_moving_ == true) {
    ...
    for (int id = 1; id <= MAX_JOINT_ID; id++)
      joint_state_->goal_joint_state_[id].position_ =
          base_module_state_->calc_joint_tra_(base_module_state_->cnt_, id);
  ...
  if ((base_module_state_->cnt_ >= base_module_state_->all_time_steps_) &&
      (base_module_state_->is_moving_ == true)) {
...
void BaseModule::stop() { return; }

void BaseModule::onModuleEnable() { ROS_INFO("Base Module is enabled"); }

void BaseModule::onModuleDisable() { has_goal_joints_ = false; }
