...
void WalkingModule::initialize(const int control_cycle_msec,
  ...
  std::string default_param_path =
      ros::package::getPath("op3_walking_module") + "/config/param.yaml";
...
bool WalkingModule::computeLegAngle(double *leg_angle) {
  ...
  double leg_length = op3_kd_->thigh_length_m_ + op3_kd_->calf_length_m_ +
                      op3_kd_->ankle_length_m_;
  ...
  // right leg
  if (op3_kd_->calcInverseKinematicsForRightLeg(
  ...
  if (op3_kd_->calcInverseKinematicsForLeftLeg(
