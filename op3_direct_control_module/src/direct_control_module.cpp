...
void DirectControlModule::setJointCallback(
  ...
  if (check_collision_ == true) {
    ...
    OP3KinematicsDynamics *op3_kinematics_for_target =
    ...
    bool result = getDiff(op3_kinematics_for_target, RIGHT_END_EFFECTOR_INDEX,
                          BASE_INDEX, diff_length);
    if (result == true && diff_length < 0.075)
      will_be_collision_ = true;

    diff_length = 0.0;
    result = getDiff(op3_kinematics_for_target, LEFT_END_EFFECTOR_INDEX,
                     BASE_INDEX, diff_length);
    if (result == true && diff_length < 0.075)
      will_be_collision_ = true;
  }
...
bool DirectControlModule::checkSelfCollision() {
  ...
  double diff_length = 0.0;
  bool result = getDiff(op3_kinematics_, RIGHT_END_EFFECTOR_INDEX, BASE_INDEX,
                        diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(op3_kinematics_, LEFT_END_EFFECTOR_INDEX, BASE_INDEX,
                   diff_length);
...
bool DirectControlModule::getDiff(OP3KinematicsDynamics *kinematics,
  ...
  if (kinematics->op3_link_data_[end_index] == NULL |
      kinematics->op3_link_data_[base_index] == NULL)
    return false;

  Eigen::Vector3d end_position =
      kinematics->op3_link_data_[end_index]->position_;
  Eigen::Vector3d base_position =
      kinematics->op3_link_data_[base_index]->position_;
