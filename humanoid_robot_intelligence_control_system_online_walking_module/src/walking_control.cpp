...
#include "humanoid_robot_intelligence_control_system_online_walking_module/walking_control.h"
#include <stdio.h>

WalkingControl::WalkingControl(double control_cycle, double dsp_ratio,
                               double lipm_height, double foot_height_max,
                               double zmp_offset_x, double zmp_offset_y,
...
void WalkingControl::initialize(
    humanoid_robot_intelligence_control_system_online_walking_module_msgs::FootStepCommand foot_step_command,
  ...
  Eigen::Quaterniond body_Q(init_body_Q[3], init_body_Q[0], init_body_Q[1],
                            init_body_Q[2]);
  init_body_Q_ = body_Q;
  des_body_Q_ = body_Q;

  Eigen::MatrixXd init_body_rpy =
      humanoid_robot_intelligence_control_system_framework::convertQuaternionToRPY(init_body_Q_);
  ...
  Eigen::Quaterniond l_foot_Q(init_l_foot_Q[3], init_l_foot_Q[0],
                              init_l_foot_Q[1], init_l_foot_Q[2]);
  init_l_foot_Q_ = l_foot_Q;
  des_l_foot_Q_ = l_foot_Q;

  Eigen::Quaterniond r_foot_Q(init_r_foot_Q[3], init_r_foot_Q[0],
                              init_r_foot_Q[1], init_r_foot_Q[2]);
...
void WalkingControl::initialize(
    humanoid_robot_intelligence_control_system_online_walking_module_msgs::Step2DArray foot_step_2d,
    std::vector<double_t> init_body_pos, std::vector<double_t> init_body_Q,
    std::vector<double_t> init_r_foot_pos, std::vector<double_t> init_r_foot_Q,
    std::vector<double_t> init_l_foot_pos,
    std::vector<double_t> init_l_foot_Q) {
...
double WalkingControl::getLipmHeight() { return lipm_height_; }
...
void WalkingControl::calcFootStepParam() {
  ...
  for (int i = 0; i < foot_step_size_; i++) {
    ...
    if ((foot_step_command_.command == "forward" ||
         foot_step_command_.command == "backward") &&
    ...
    if (i == 0 || i == 1 || i == foot_step_size_ - 2 ||
        i == foot_step_size_ - 1) {
      msg.x = 0.0;
...
void WalkingControl::transformFootStep2D() {
  ...
  humanoid_robot_intelligence_control_system_online_walking_module_msgs::FootStepArray foot_step_param;

  for (int step = 0; step < foot_step_size_; step++) {
    humanoid_robot_intelligence_control_system_online_walking_module_msgs::Step2D msg =
...
void WalkingControl::calcFootTrajectory(int step) {
  Eigen::MatrixXd body_rot =
      humanoid_robot_intelligence_control_system_framework::convertQuaternionToRotation(des_body_Q_);
  Eigen::MatrixXd left_foot_rot =
      humanoid_robot_intelligence_control_system_framework::convertQuaternionToRotation(des_l_foot_Q_);
  Eigen::MatrixXd r_foot_rot =
      humanoid_robot_intelligence_control_system_framework::convertQuaternionToRotation(des_r_foot_Q_);

  init_body_Q_ = humanoid_robot_intelligence_control_system_framework::convertRotationToQuaternion(body_rot);
  init_l_foot_Q_ =
      humanoid_robot_intelligence_control_system_framework::convertRotationToQuaternion(left_foot_rot);
  init_r_foot_Q_ = humanoid_robot_intelligence_control_system_framework::convertRotationToQuaternion(r_foot_rot);
  ...
  if (foot_step_param_.moving_foot[step] == LEFT_LEG) {
    ...
    goal_l_foot_Q_ = humanoid_robot_intelligence_control_system_framework::convertRPYToQuaternion(0.0, 0.0, angle);

    goal_r_foot_pos_ = init_r_foot_pos_;
    goal_r_foot_Q_ = init_r_foot_Q_;

    goal_body_Q_ = humanoid_robot_intelligence_control_system_framework::convertRPYToQuaternion(0.0, 0.0, angle);
    ...
    // Trajectory
    l_foot_tra_ = new humanoid_robot_intelligence_control_system_framework::MinimumJerkViaPoint(
    ...
    goal_r_foot_Q_ = humanoid_robot_intelligence_control_system_framework::convertRPYToQuaternion(0.0, 0.0, angle);
    ...
    // Trajectory
    r_foot_tra_ = new humanoid_robot_intelligence_control_system_framework::MinimumJerkViaPoint(
...
void WalkingControl::calcRefZMP(int step) {
  if (step == 0 || step == 1) {
    ref_zmp_x_ =
        0.5 * (goal_r_foot_pos_[0] + goal_l_foot_pos_[0]); // + zmp_offset_x_;
...
double WalkingControl::calcRefZMPx(int step) {
  ...
  if (step == 0 || step == 1) {
    ref_zmp_x =
        0.5 * (goal_r_foot_pos_buffer_.coeff(step, 0) +
               goal_l_foot_pos_buffer_.coeff(step, 0)); // + zmp_offset_x_;
  } else if (step >= foot_step_size_ - 1) {
...
double WalkingControl::calcRefZMPy(int step) {
  ...
  if (step == 0 || step == 1) {
    ref_zmp_y = 0.5 * (goal_r_foot_pos_buffer_.coeff(step, 1) +
                       goal_l_foot_pos_buffer_.coeff(step, 1));
  } else if (step >= foot_step_size_ - 1) {
...
void WalkingControl::calcPreviewParam(std::vector<double_t> K, int K_row,
                                      int K_col, std::vector<double_t> P,
                                      int P_row, int P_col) {
  //
  ...
  preview_control_ = new humanoid_robot_intelligence_control_system_framework::PreviewControl();
...
void WalkingControl::calcPreviewControl(double time, int step) {
  ...
  u_x_(0, 0) = -k_s_ * (sum_of_cx_ - sum_of_zmp_x_) -
               (k_x_(0, 0) * x_lipm_(0, 0) + k_x_(0, 1) * x_lipm_(1, 0) +
                k_x_(0, 2) * x_lipm_(2, 0)) +
               preview_sum_zmp_x_;
  u_y_(0, 0) = -k_s_ * (sum_of_cy_ - sum_of_zmp_y_) -
               (k_x_(0, 0) * y_lipm_(0, 0) + k_x_(0, 1) * y_lipm_(1, 0) +
                k_x_(0, 2) * y_lipm_(2, 0)) +
               preview_sum_zmp_y_;

  x_lipm_ = A_ * x_lipm_ + b_ * u_x_;
  y_lipm_ = A_ * y_lipm_ + b_ * u_y_;

  double cx = c_(0, 0) * x_lipm_(0, 0) + c_(0, 1) * x_lipm_(1, 0) +
              c_(0, 2) * x_lipm_(2, 0);
  double cy = c_(0, 0) * y_lipm_(0, 0) + c_(0, 1) * y_lipm_(1, 0) +
              c_(0, 2) * y_lipm_(2, 0);
