...
#include "humanoid_robot_intelligence_control_system_direct_control_module/direct_control_module.h"
#include <stdio.h>
...
DirectControlModule::DirectControlModule()
    : control_cycle_msec_(0), stop_process_(false), is_moving_(false),
      is_updated_(false), is_blocked_(false), r_min_diff_(0.07),
      l_min_diff_(0.07), tra_count_(0), tra_size_(0), default_moving_time_(0.5),
      default_moving_angle_(30), check_collision_(true), moving_time_(3.0),
      BASE_INDEX(0), HEAD_INDEX(20), RIGHT_END_EFFECTOR_INDEX(21),
      RIGHT_ELBOW_INDEX(5), LEFT_END_EFFECTOR_INDEX(22), LEFT_ELBOW_INDEX(6),
      DEBUG(false) {
  enable_ = false;
...
void DirectControlModule::initialize(const int control_cycle_msec,
  ...
  humanoid_robot_intelligence_control_system_kinematics_ = new HUMANOID_ROBOTKinematicsDynamics(WholeBody);

  // init result, joint_id_table
  int joint_index = 0;
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::Dynamixel *>::iterator it =
           robot->dxls_.begin();
       it != robot->dxls_.end(); it++) {
    std::string joint_name = it->first;
    humanoid_robot_intelligence_control_system_framework::Dynamixel *dxl_info = it->second;

    result_[joint_name] = new humanoid_robot_intelligence_control_system_framework::DynamixelState();
  ...
  queue_thread_ =
      boost::thread(boost::bind(&DirectControlModule::queueThread, this));
  ...
  /* get Param */
  ros_node.param<double>("/humanoid_robot_intelligence_control_system/direct_control/default_moving_time",
                         default_moving_time_, default_moving_time_);
  ros_node.param<double>("/humanoid_robot_intelligence_control_system/direct_control/default_moving_angle",
                         default_moving_angle_, default_moving_angle_);
  ros_node.param<bool>("/humanoid_robot_intelligence_control_system/direct_control/check_collision",
  ...
  status_msg_pub_ = ros_node.advertise<humanoid_robot_intelligence_control_system_controller_msgs::StatusMsg>(
      "/humanoid_robot_intelligence_control_system/status", 0);
}
...
void DirectControlModule::queueThread() {
  ...
  ros::Subscriber set_head_joint_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/direct_control/set_joint_states", 1,
...
void DirectControlModule::setJointCallback(
    const sensor_msgs::JointState::ConstPtr &msg) {
  if (enable_ == false) {
    ROS_INFO_THROTTLE(1, "Direct control module is not enable.");
    publishStatusMsg(humanoid_robot_intelligence_control_system_controller_msgs::StatusMsg::STATUS_ERROR,
                     "Not Enable");
    return;
  }
  ...
  if (is_moving_ == true) {
    goal_velocity_ =
        calc_joint_vel_tra_.block(tra_count_, 0, 1, result_.size());
    goal_acceleration_ =
        calc_joint_accel_tra_.block(tra_count_, 0, 1, result_.size());
  }
  ...
  for (int ix = 0; ix < msg->name.size(); ix++) {
    ...
    std::map<std::string, int>::iterator joint_it =
    ...
    if (joint_it != using_joint_name_.end()) {
      ...
      double calc_moving_time = fabs(goal_position_.coeff(0, joint_index) -
  ...
  if (check_collision_ == true) {
    ...
    HUMANOID_ROBOTKinematicsDynamics *humanoid_robot_intelligence_control_system_kinematics_for_target =
    ...
    for (std::map<std::string, int>::iterator joint_index_it =
      ...
      LinkData *humanoid_robot_intelligence_control_system_link = humanoid_robot_intelligence_control_system_kinematics_for_target->getLinkData(joint_name);
      if (humanoid_robot_intelligence_control_system_link != NULL)
        humanoid_robot_intelligence_control_system_link->joint_angle_ = target_position;
    }

    humanoid_robot_intelligence_control_system_kinematics_for_target->calcForwardKinematics(0);

    double diff_length = 0.0;
    bool result = getDiff(humanoid_robot_intelligence_control_system_kinematics_for_target, RIGHT_END_EFFECTOR_INDEX,
                          BASE_INDEX, diff_length);
    if (result == true && diff_length < 0.075)
      will_be_collision_ = true;

    diff_length = 0.0;
    result = getDiff(humanoid_robot_intelligence_control_system_kinematics_for_target, LEFT_END_EFFECTOR_INDEX,
                     BASE_INDEX, diff_length);
    if (result == true && diff_length < 0.075)
      will_be_collision_ = true;
  }
  ...
  tra_gene_thread_ = new boost::thread(
      boost::bind(&DirectControlModule::jointTraGeneThread, this));
...
void DirectControlModule::process(
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    humanoid_robot_intelligence_control_system_framework::Dynamixel *_dxl = NULL;
  ...
  if (stop_process_ == true) {
    ...
    if (tra_size_ != 0) {
      ...
      if (tra_count_ >= tra_size_) {
        ...
        goal_position_ =
            calc_joint_tra_.block(tra_count_, 0, 1, result_.size());
        tra_count_ += 1;
      }
    }
  }
  ...
  if (check_collision_ == true) {
    ...
    for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
      ...
      LinkData *humanoid_robot_intelligence_control_system_link = humanoid_robot_intelligence_control_system_kinematics_->getLinkData(joint_name);
      if (humanoid_robot_intelligence_control_system_link != NULL)
        humanoid_robot_intelligence_control_system_link->joint_angle_ = goal_position;
    }

    humanoid_robot_intelligence_control_system_kinematics_->calcForwardKinematics(0);
...
bool DirectControlModule::isRunning() { return is_moving_; }
...
void DirectControlModule::stopMoving() {
  ...
  // log
  publishStatusMsg(humanoid_robot_intelligence_control_system_controller_msgs::StatusMsg::STATUS_WARN,
                   "Stop Module.");
}
 ...
 */
Eigen::MatrixXd DirectControlModule::calcMinimumJerkTraPVA(
    double pos_start, double vel_start, double accel_start, double pos_end,
  ...
  poly_matrix << humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 3),
      humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 4),
      humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 5),
      3 * humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 2),
      4 * humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 3),
      5 * humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 4), 6 * mov_time,
      12 * humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 2),
      20 * humanoid_robot_intelligence_control_system_framework::powDI(mov_time, 3);

  poly_vector << pos_end - pos_start - vel_start * mov_time -
                     accel_start * pow(mov_time, 2) / 2,
      vel_end - vel_start - accel_start * mov_time, accel_end - accel_start;
  ...
  for (int step = 0; step < all_time_steps; step++) {
    ...
    minimum_jer_tra.coeffRef(step, 0) =
        ...
        0.5 * accel_start * humanoid_robot_intelligence_control_system_framework::powDI(time.coeff(step, 0), 2) +
        poly_coeff.coeff(0, 0) *
            humanoid_robot_intelligence_control_system_framework::powDI(time.coeff(step, 0), 3) +
        poly_coeff.coeff(1, 0) *
            humanoid_robot_intelligence_control_system_framework::powDI(time.coeff(step, 0), 4) +
        poly_coeff.coeff(2, 0) *
            humanoid_robot_intelligence_control_system_framework::powDI(time.coeff(step, 0), 5);
    // velocity
    minimum_jer_tra.coeffRef(step, 1) =
        vel_start + accel_start * time.coeff(step, 0) +
        3 * poly_coeff.coeff(0, 0) *
            humanoid_robot_intelligence_control_system_framework::powDI(time.coeff(step, 0), 2) +
...
bool DirectControlModule::checkSelfCollision() {
  ...
  double diff_length = 0.0;
  bool result = getDiff(humanoid_robot_intelligence_control_system_kinematics_, RIGHT_END_EFFECTOR_INDEX, BASE_INDEX,
                        diff_length);

  // check collision
  if (result == true && diff_length < collision_boundary) {
    if (DEBUG == true)
      ROS_WARN_STREAM_THROTTLE(1, "Self Collision : RIGHT_ARM and BASE | "
                                      << diff_length << " / "
                                      << collision_boundary);
  ...
  diff_length = 0.0;
  result = getDiff(humanoid_robot_intelligence_control_system_kinematics_, RIGHT_ELBOW_INDEX, BASE_INDEX, diff_length);

  // check collision
  if (result == true && diff_length < collision_boundary) {
    if (DEBUG == true)
      ROS_WARN_STREAM_THROTTLE(1, "Self Collision : RIGHT_ELBOW and BASE | "
                                      << diff_length << " / "
                                      << collision_boundary);
    collision_["r_sho_pitch"] = true;
    collision_["r_sho_roll"] = true;
  ...
  diff_length = 0.0;
  result = getDiff(humanoid_robot_intelligence_control_system_kinematics_, LEFT_END_EFFECTOR_INDEX, BASE_INDEX,
                   diff_length);

  // check collision
  if (result == true && diff_length < collision_boundary) {
    if (DEBUG == true)
      ROS_WARN_STREAM_THROTTLE(1, "Self Collision : LEFT_ARM and BASE | "
                                      << diff_length << " / "
                                      << collision_boundary);
  ...
  diff_length = 0.0;
  result = getDiff(humanoid_robot_intelligence_control_system_kinematics_, LEFT_ELBOW_INDEX, BASE_INDEX, diff_length);

  // check collision
  if (result == true && diff_length < collision_boundary) {
    if (DEBUG == true)
      ROS_WARN_STREAM_THROTTLE(1, "Self Collision : LEFT_ELBOW and BASE | "
                                      << diff_length << " / "
                                      << collision_boundary);
    collision_["l_sho_pitch"] = true;
    collision_["l_sho_roll"] = true;
...
bool DirectControlModule::getDiff(HUMANOID_ROBOTKinematicsDynamics *kinematics,
  ...
  if (kinematics->humanoid_robot_intelligence_control_system_link_data_[end_index] == NULL |
      kinematics->humanoid_robot_intelligence_control_system_link_data_[base_index] == NULL)
    return false;

  Eigen::Vector3d end_position =
      kinematics->humanoid_robot_intelligence_control_system_link_data_[end_index]->position_;
  Eigen::Vector3d base_position =
      kinematics->humanoid_robot_intelligence_control_system_link_data_[base_index]->position_;
...
void DirectControlModule::jointTraGeneThread() {
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    Eigen::MatrixXd tra =
        calcMinimumJerkTraPVA(ini_pos, ini_vel, ini_accel, tar_value, 0.0, 0.0,
                              smp_time, moving_time_);

    calc_joint_tra_.block(0, index, all_time_steps, 1) =
        tra.block(0, Position, all_time_steps, 1);
    calc_joint_vel_tra_.block(0, index, all_time_steps, 1) =
        tra.block(0, Velocity, all_time_steps, 1);
    calc_joint_accel_tra_.block(0, index, all_time_steps, 1) =
        tra.block(0, Acceleration, all_time_steps, 1);
  }
