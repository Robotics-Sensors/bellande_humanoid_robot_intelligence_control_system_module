...
void DirectControlModule::setJointCallback(
  ...
  if (check_collision_ == true) {
    ...
    for (std::map<std::string, int>::iterator joint_index_it =
      ...
      LinkData *humanoid_robot_link = humanoid_robot_kinematics_for_target->getLinkData(joint_name);
      if (humanoid_robot_link != NULL)
        humanoid_robot_link->joint_angle_ = target_position;
    }

    humanoid_robot_kinematics_for_target->calcForwardKinematics(0);

    double diff_length = 0.0;
    bool result = getDiff(humanoid_robot_kinematics_for_target, RIGHT_END_EFFECTOR_INDEX,
                          BASE_INDEX, diff_length);
    if (result == true && diff_length < 0.075)
      will_be_collision_ = true;

    diff_length = 0.0;
    result = getDiff(humanoid_robot_kinematics_for_target, LEFT_END_EFFECTOR_INDEX,
                     BASE_INDEX, diff_length);
    if (result == true && diff_length < 0.075)
      will_be_collision_ = true;
  }
...
void DirectControlModule::process(
  ...
  if (check_collision_ == true) {
    ...
    for (std::map<std::string, humanoid_robot_framework::DynamixelState *>::iterator
      ...
      LinkData *humanoid_robot_link = humanoid_robot_kinematics_->getLinkData(joint_name);
      if (humanoid_robot_link != NULL)
        humanoid_robot_link->joint_angle_ = goal_position;
    }

    humanoid_robot_kinematics_->calcForwardKinematics(0);
...
bool DirectControlModule::checkSelfCollision() {
  ...
  double diff_length = 0.0;
  bool result = getDiff(humanoid_robot_kinematics_, RIGHT_END_EFFECTOR_INDEX, BASE_INDEX,
                        diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(humanoid_robot_kinematics_, RIGHT_ELBOW_INDEX, BASE_INDEX, diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(humanoid_robot_kinematics_, LEFT_END_EFFECTOR_INDEX, BASE_INDEX,
                   diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(humanoid_robot_kinematics_, LEFT_ELBOW_INDEX, BASE_INDEX, diff_length);
...
bool DirectControlModule::getDiff(HUMANOID_ROBOTKinematicsDynamics *kinematics,
  ...
  Eigen::Vector3d end_position =
      kinematics->humanoid_robot_link_data_[end_index]->position_;
  Eigen::Vector3d base_position =
      kinematics->humanoid_robot_link_data_[base_index]->position_;
