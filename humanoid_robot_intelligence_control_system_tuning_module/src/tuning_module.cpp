...
#include "humanoid_robot_intelligence_control_system_tuning_module/tuning_module.h"
#include <stdio.h>
...
void TuningModule::initialize(const int control_cycle_msec,
  ...
  set_ctrl_module_pub_ =
      ros_node.advertise<std_msgs::String>("/humanoid_robot_intelligence_control_system/enable_ctrl_module", 1);
  sync_write_pub_ = ros_node.advertise<humanoid_robot_intelligence_control_system_controller_msgs::SyncWriteItem>(
      "/humanoid_robot_intelligence_control_system/sync_write_item", 1);
...
void TuningModule::moveToInitPose() {
  ...
  tra_gene_tread_ = boost::thread(
      boost::bind(&TuningModule::targetPoseTrajGenerateProc, this));
}

void TuningModule::moveToTunePose(const std::string &pose_name) {
  if (enable_ == false) {
    ROS_ERROR("humanoid_robot_intelligence_control_system_tuning_module is not enable!!");
    return;
  }
...
bool TuningModule::parseOffsetData(const std::string &path) {
  ...
  for (YAML::const_iterator node_it = offset_data_node.begin();
    ...
    std::map<std::string, JointOffsetData *>::iterator robot_tuning_data_it =
        robot_tuning_data_.find(joint_name);
...
bool TuningModule::parseInitPoseData(const std::string &path) {
  ...
  for (YAML::iterator yaml_it = tar_pose_node.begin();
       yaml_it != tar_pose_node.end(); ++yaml_it) {
    ...
    tuning_module_state_->joint_ini_pose_.coeffRef(id, 0) =
        value * DEGREE2RADIAN;
  ...
  tuning_module_state_->all_time_steps_ =
      int(tuning_module_state_->mov_time_ / tuning_module_state_->smp_time_) +
      1;
  tuning_module_state_->calc_joint_tra_.resize(
      tuning_module_state_->all_time_steps_, MAX_JOINT_ID + 1);

  return true;
}

bool TuningModule::parseTunePoseData(const std::string &path,
  ...
  target_pose_name =
      tune_pose_node["target_pose"].as<std::vector<std::string>>();
  ...
  for (int num = 0; num < via_num; num++) {
    ...
    for (YAML::iterator yaml_it = via_pose_node.begin();
         yaml_it != via_pose_node.end(); ++yaml_it) {
      std::string joint_name;
      double value;
      ...
      tuning_module_state_->joint_via_pose_.coeffRef(num, id) =
          value * DEGREE2RADIAN;
  ...
  ROS_INFO_STREAM("tune pose - via_num : " << via_num << ", move_time : "
                                           << total_move_time);
...
void TuningModule::queueThread() {
  ...
  ros::Subscriber ini_pose_msg_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/tuning_module/tuning_pose", 5,
                         &TuningModule::tunePoseMsgCallback, this);

  joint_offset_data_sub_ =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/tuning_module/joint_offset_data", 10,
                         &TuningModule::jointOffsetDataCallback, this);
  joint_gain_data_sub_ =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/tuning_module/joint_gain_data", 10,
                         &TuningModule::jointGainDataCallback, this);
  joint_torque_enable_sub_ =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/tuning_module/torque_enable", 10,
                         &TuningModule::jointTorqueOnOffCallback, this);
  command_sub_ = ros_node.subscribe("/humanoid_robot_intelligence_control_system/tuning_module/command", 5,
                                    &TuningModule::commandCallback, this);
  offset_data_server_ = ros_node.advertiseService(
      "humanoid_robot_intelligence_control_system/tuning_module/get_present_joint_offset_data",
      &TuningModule::getPresentJointOffsetDataServiceCallback, this);

  set_module_client_ =
      ros_node.serviceClient<humanoid_robot_intelligence_control_system_controller_msgs::SetModule>(
          "/humanoid_robot_intelligence_control_system/set_present_ctrl_modules");
  enable_offset_pub_ =
      ros_node.advertise<std_msgs::Bool>("/humanoid_robot_intelligence_control_system/enable_offset", 1);
  load_offset_client_ =
      ros_node.serviceClient<humanoid_robot_intelligence_control_system_controller_msgs::LoadOffset>(
...
void TuningModule::targetPoseTrajGenerateProc() {
  for (int id = 1; id <= MAX_JOINT_ID; id++) {
    ...
    if (tuning_module_state_->via_num_ == 0) {
      tra = humanoid_robot_intelligence_control_system_framework::calcMinimumJerkTra(
      ...
      Eigen::MatrixXd d_via_value =
          tuning_module_state_->joint_via_dpose_.col(id);
      Eigen::MatrixXd dd_via_value =
          tuning_module_state_->joint_via_ddpose_.col(id);

      tra = humanoid_robot_intelligence_control_system_framework::calcMinimumJerkTraWithViaPoints(
    ...
    tuning_module_state_->calc_joint_tra_.block(
        0, id, tuning_module_state_->all_time_steps_, 1) = tra;
  }
...
void TuningModule::poseGenerateProc(Eigen::MatrixXd joint_angle_pose) {
  ...
  for (int id = 1; id <= MAX_JOINT_ID; id++) {
    ...
    Eigen::MatrixXd tra = humanoid_robot_intelligence_control_system_framework::calcMinimumJerkTra(
        ini_value, 0.0, 0.0, tar_value, 0.0, 0.0,
        tuning_module_state_->smp_time_, tuning_module_state_->mov_time_);
...
void TuningModule::poseGenerateProc(
    std::map<std::string, double> &joint_angle_pose) {
  callServiceSettingModule(module_name_);
  ...
  for (std::map<std::string, double>::iterator joint_angle_it =
           joint_angle_pose.begin();
    ...
    std::map<std::string, int>::iterator joint_name_to_id_it =
...
void TuningModule::process(
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    double joint_pres_position =
        dxl->dxl_state_->present_position_ - offset_value;
    ...
    joint_state_->curr_joint_state_[joint_name_to_id_[joint_name]].position_ =
        joint_pres_position;
    joint_state_->curr_joint_state_[joint_name_to_id_[joint_name]].p_gain_ =
        p_gain;
    joint_state_->curr_joint_state_[joint_name_to_id_[joint_name]].i_gain_ =
        i_gain;
    joint_state_->curr_joint_state_[joint_name_to_id_[joint_name]].d_gain_ =
        d_gain;

    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].position_ =
        joint_goal_position;
    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].p_gain_ =
        p_gain;
    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].i_gain_ =
        i_gain;
    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].d_gain_ =
        d_gain;
  }
  ...
  // if has tuning data
    ...
    if (data_mutex_.try_lock() == true) {
      ...
      if (joint_name_to_id_it != joint_name_to_id_.end()) {
        ...
        tuning_data_.position_.getValue(
            joint_state_->goal_joint_state_[joint_id].position_);

        tuning_data_.p_gain_.getValue(
            joint_state_->goal_joint_state_[joint_id].p_gain_);
        tuning_data_.i_gain_.getValue(
            joint_state_->goal_joint_state_[joint_id].i_gain_);
        tuning_data_.d_gain_.getValue(
            joint_state_->goal_joint_state_[joint_id].d_gain_);
      }
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    if (robot_torque_enable_data_[joint_name] == false) {
      robot_tuning_data_[joint_name]->joint_offset_rad_ =
          joint_state_->curr_joint_state_[joint_name_to_id_[joint_name]]
              .position_ -
    ...
    result_[joint_name]->goal_position_ =
        joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]]
            .position_ +
        offset_value;
    robot_tuning_data_[joint_name]->goal_position_ =
        joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]]
  ...
  if ((tuning_module_state_->cnt_ >= tuning_module_state_->all_time_steps_) &&
      (tuning_module_state_->is_moving_ == true)) {
...
void TuningModule::commandCallback(const std_msgs::String::ConstPtr &msg) {
  if (msg->data == "save_offset") {
    ...
    // send a command to humanoid_robot_intelligence_control_system_controller to load the new offset file
...
void TuningModule::jointOffsetDataCallback(
    const humanoid_robot_intelligence_control_system_tuning_module_msgs::JointOffsetData::ConstPtr &msg) {
...
void TuningModule::jointGainDataCallback(
    const humanoid_robot_intelligence_control_system_tuning_module_msgs::JointOffsetData::ConstPtr &msg) {
  if (this->enable_ == false) {
    ROS_ERROR("Tuning module is not enable");
    return;
...
void TuningModule::jointTorqueOnOffCallback(
    const humanoid_robot_intelligence_control_system_tuning_module_msgs::JointTorqueOnOffArray::ConstPtr &msg) {
...
bool TuningModule::getPresentJointOffsetDataServiceCallback(
    humanoid_robot_intelligence_control_system_tuning_module_msgs::GetPresentJointOffsetData::Request &req,
    humanoid_robot_intelligence_control_system_tuning_module_msgs::GetPresentJointOffsetData::Response &res) {

  ROS_INFO("GetPresentJointOffsetDataService Called");

  for (std::map<std::string, JointOffsetData *>::iterator map_it =
           robot_tuning_data_.begin();
       map_it != robot_tuning_data_.end(); map_it++) {
    std::string joint_name = map_it->first;
    JointOffsetData *joint_data = map_it->second;

    humanoid_robot_intelligence_control_system_tuning_module_msgs::JointOffsetPositionData joint_offset_pos;
    ...
    joint_offset_pos.present_value =
        present_value; // present value of this joint
...
bool TuningModule::turnOnOffOffset(bool turn_on) {
  ROS_WARN("Try to turn on/off offset in humanoid_robot_intelligence_control_system_controller");
...
bool TuningModule::loadOffsetToController(const std::string &path) {
  humanoid_robot_intelligence_control_system_controller_msgs::LoadOffset load_offset_srv;
  ...
  if (load_offset_client_.call(load_offset_srv) == true) {
    if (load_offset_srv.response.result == true)
      ROS_INFO("succeed to let humanoid_robot_intelligence_control_system_controller load the offset");
    else
      ROS_ERROR("Failed to let humanoid_robot_intelligence_control_system_controller load the offset");
...
void TuningModule::saveOffsetToYaml(const std::string &path) {
  ...
  for (std::map<std::string, JointOffsetData *>::iterator map_it =
    ...
    //    init_pose[joint_name] = joint_data->joint_init_pos_rad_;  // edit one
    //    of the nodes
  }

  yaml_out << YAML::BeginMap;
  yaml_out << YAML::Key << "offset" << YAML::Value << offset;
  //  yaml_out << YAML::Key << "init_pose_for_tuning_module" << YAML::Value <<
  //  init_pose;
...
void TuningModule::saveDxlInit(const std::string &path) {
  ...
  for (std::map<std::string, JointOffsetData *>::iterator map_it =
    ...
    for (YAML::const_iterator node_it = joint_node.begin();
         node_it != joint_node.end(); node_it++) {
