...
#include "humanoid_robot_intelligence_control_system_walking_module/humanoid_robot_intelligence_control_system_walking_module.h"
...
WalkingModule::WalkingModule() : control_cycle_msec_(8), DEBUG(false) {
  ...
  humanoid_robot_intelligence_control_system_kd_ = new HUMANOID_ROBOTKinematicsDynamics(WholeBody);
  ...
  result_["r_sho_pitch"] = new humanoid_robot_intelligence_control_system_framework::DynamixelState();
  result_["l_sho_pitch"] = new humanoid_robot_intelligence_control_system_framework::DynamixelState();
...
void WalkingModule::initialize(const int control_cycle_msec,
  ...
  //                     R_ANKLE_PITCH, R_ANKLE_ROLL, L_HIP_YAW, L_HIP_ROLL,
  //                     L_HIP_PITCH, L_KNEE, L_ANKLE_PITCH, L_ANKLE_ROLL,
  ...
  std::string default_param_path =
      ros::package::getPath("humanoid_robot_intelligence_control_system_walking_module") + "/config/param.yaml";
  ros_node.param<std::string>("walking_param_path", param_path_,
                              default_param_path);
...
void WalkingModule::queueThread() {
  ...
  ros::ServiceServer get_walking_param_server = ros_node.advertiseService(
      "/humanoid_robot_intelligence_control_system/walking/get_params", &WalkingModule::getWalkigParameterCallback,
      this);

  /* sensor topic subscribe */
  ros::Subscriber walking_command_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/walking/command", 0,
                         &WalkingModule::walkingCommandCallback, this);
  ros::Subscriber walking_param_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/walking/set_params", 0,
...
void WalkingModule::publishStatusMsg(unsigned int type, std::string msg) {
  humanoid_robot_intelligence_control_system_controller_msgs::StatusMsg status_msg;
...
void WalkingModule::walkingCommandCallback(
...
void WalkingModule::walkingParameterCallback(
    const humanoid_robot_intelligence_control_system_walking_module_msgs::WalkingParam::ConstPtr &msg) {
  walking_param_ = *msg;
}

bool WalkingModule::getWalkigParameterCallback(
    humanoid_robot_intelligence_control_system_walking_module_msgs::GetWalkingParam::Request &req,
    humanoid_robot_intelligence_control_system_walking_module_msgs::GetWalkingParam::Response &res) {
...
// m, rad
// for default humanoid_robot_intelligence_control_system: it was used from previos version(OP2) but it's not using
// now.
bool WalkingModule::computeIK(double *out, double pos_x, double pos_y,
                              double pos_z, double ori_roll, double ori_pitch,
                              double ori_yaw) {
  double thigh_length = 93.0 * 0.001; // m
  double calf_length = 93.0 * 0.001;  // m
  double ankle_length = 33.5 * 0.001; // m
  double leg_length =
      219.5 * 0.001; // m (THIGH_LENGTH + CALF_LENGTH + ANKLE_LENGTH)

  Eigen::MatrixXd transformation_ad, transformation_da, transformation_cd,
      transformation_dc, transformation_ac;
  Eigen::Vector3d vector;
  double r_ac, acos_value, atan_value, value_k, value_l, value_m, value_n,
      value_s, value_c, theta;

  // make transform matrix
  transformation_ad = humanoid_robot_intelligence_control_system_framework::getTransformationXYZRPY(
  ...
  r_ac = vector.norm();
  acos_value = acos(
      (r_ac * r_ac - thigh_length * thigh_length - calf_length * calf_length) /
  ...
  *(out + 3) = acos_value;
  ...
  // Get Ankle Roll
  transformation_da =
      humanoid_robot_intelligence_control_system_framework::getInverseTransformation(transformation_ad);
  ...
  value_m =
      (value_k * value_k - value_l * value_l - ankle_length * ankle_length) /
      (2 * value_l * ankle_length);
  if (value_m > 1.0)
    value_m = 1.0;
  else if (value_m < -1.0)
    value_m = -1.0;
  ...
  // Get Hip Yaw
  transformation_cd = humanoid_robot_intelligence_control_system_framework::getTransformationXYZRPY(
      0.0, 0.0, -ankle_length, *(out + 5), 0.0, 0.0);
  transformation_dc =
      humanoid_robot_intelligence_control_system_framework::getInverseTransformation(transformation_cd);
  ...
  *(out) = atan_value;
  ...
  atan_value = atan2(transformation_ac.coeff(2, 1),
                     -transformation_ac.coeff(0, 1) * sin(*(out)) +
                         transformation_ac.coeff(1, 1) * cos(*(out)));
  if (std::isinf(atan_value) == 1)
    return false;
  *(out + 1) = atan_value;

  // Get Hip Pitch and Ankle Pitch
  atan_value = atan2(transformation_ac.coeff(0, 2) * cos(*(out)) +
                         transformation_ac.coeff(1, 2) * sin(*(out)),
                     transformation_ac.coeff(0, 0) * cos(*(out)) +
                         transformation_ac.coeff(1, 0) * sin(*(out)));
  if (std::isinf(atan_value) == 1)
    return false;
  theta = atan_value;
  ...
  value_n = cos(*(out + 1)) * vector.z() +
            sin(*(out)) * sin(*(out + 1)) * vector.x() -
            cos(*(out)) * sin(*(out + 1)) * vector.y();
  value_s = (value_k * value_n + value_l * value_m) /
            (value_k * value_k + value_l * value_l);
...
void WalkingModule::updateMovementParam() {
  ...
  x_swap_amplitude_ =
      walking_param_.x_move_amplitude * walking_param_.step_fb_ratio;
  ...
  if (y_move_amplitude_ > 0)
    ...
    y_move_amplitude_shift_ = -y_move_amplitude_;
  y_swap_amplitude_ =
      walking_param_.y_swap_amplitude + y_move_amplitude_shift_ * 0.04;
...
void WalkingModule::startWalking() {
  ...
  publishStatusMsg(humanoid_robot_intelligence_control_system_controller_msgs::StatusMsg::STATUS_INFO,
                   "Start walking");
}
...
void WalkingModule::process(
  ...
  if (walking_state_ == WalkingInitPose) {
    ...
    for (int idx = 0; idx < 14; idx++) {
      ...
      double err =
          fabs(target_position_.coeff(0, idx) - goal_position_.coeff(0, idx)) *
          RADIAN2DEGREE;
      if (err > err_max)
        err_max = err;
      err_total += err;
    ...
    if (walking_state_ == WalkingEnable && err_total > 5.0) {
      ...
      ROS_WARN_STREAM_COND(DEBUG,
                           "roll_offset: " << walking_param_.init_roll_offset *
                                                  RADIAN2DEGREE);
      ROS_WARN_STREAM_COND(
          DEBUG,
          "pitch_offset: " << walking_param_.init_pitch_offset * RADIAN2DEGREE);
      ROS_WARN_STREAM_COND(DEBUG,
                           "yaw_offset: " << walking_param_.init_yaw_offset *
                                                 RADIAN2DEGREE);
      ROS_WARN_STREAM_COND(
          DEBUG, "hip_pitch_offset: " << walking_param_.hip_pitch_offset *
                                             RADIAN2DEGREE);
      ROS_WARN_STREAM_COND(
          DEBUG, "period_time: " << walking_param_.period_time * 1000);
      ROS_WARN_STREAM_COND(DEBUG, "dsp_ratio: " << walking_param_.dsp_ratio);
      ROS_WARN_STREAM_COND(
          DEBUG, "step_forward_back_ratio: " << walking_param_.step_fb_ratio);
      ROS_WARN_STREAM_COND(DEBUG,
                           "foot_height: " << walking_param_.z_move_amplitude);
      ROS_WARN_STREAM_COND(
          DEBUG, "swing_right_left: " << walking_param_.y_swap_amplitude);
      ROS_WARN_STREAM_COND(
          DEBUG, "swing_top_down: " << walking_param_.z_swap_amplitude);
      ROS_WARN_STREAM_COND(DEBUG,
                           "pelvis_offset: " << walking_param_.pelvis_offset *
                                                    RADIAN2DEGREE);
      ROS_WARN_STREAM_COND(DEBUG,
                           "arm_swing_gain: " << walking_param_.arm_swing_gain);
      ROS_WARN_STREAM_COND(DEBUG, "balance_hip_roll_gain: "
                                      << walking_param_.balance_hip_roll_gain);
      ROS_WARN_STREAM_COND(
          DEBUG, "balance_knee_gain: " << walking_param_.balance_knee_gain);
      ROS_WARN_STREAM_COND(DEBUG,
      ...
      ROS_WARN_STREAM_COND(
          DEBUG,
          "balance : " << (walking_param_.balance_enable ? "TRUE" : "FALSE"));
    } else {
      walking_state_ = WalkingReady;
    }
  }
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    result_[joint_name]->goal_position_ =
        target_position_.coeff(0, joint_index);
...
void WalkingModule::processPhase(const double &time_unit) {
  ...
  if (time_ == 0) {
    ...
    if (ctrl_running_ == false) {
      if (x_move_amplitude_ == 0 && y_move_amplitude_ == 0 &&
          a_move_amplitude_ == 0) {
        real_running_ = false;
      } else {
  ...
  } else if (time_ >= (phase1_time_ - time_unit / 2) &&
             time_ <
                 (phase1_time_ +
                  time_unit / 2)) // the position of left foot is the highest.
  {
    updateMovementParam();
    phase_ = PHASE1;
  } else if (time_ >= (phase2_time_ - time_unit / 2) &&
             time_ < (phase2_time_ +
                      time_unit / 2)) // middle of double support state
  {
    updateTimeParam();
  ...
  } else if (time_ >= (phase3_time_ - time_unit / 2) &&
             time_ <
                 (phase3_time_ +
                  time_unit / 2)) // the position of right foot is the highest.
  {
    updateMovementParam();
    phase_ = PHASE3;
  }
...
bool WalkingModule::computeLegAngle(double *leg_angle) {
  ...
  // Compute endpoints
  swap.x = wSin(time_, x_swap_period_time_, x_swap_phase_shift_,
                x_swap_amplitude_, x_swap_amplitude_shift_);
  swap.y = wSin(time_, y_swap_period_time_, y_swap_phase_shift_,
                y_swap_amplitude_, y_swap_amplitude_shift_);
  swap.z = wSin(time_, z_swap_period_time_, z_swap_phase_shift_,
                z_swap_amplitude_, z_swap_amplitude_shift_);
  swap.roll = 0.0;
  swap.pitch = 0.0;
  swap.yaw = 0.0;
  ...
  if (time_ <= l_ssp_start_time_) {
    left_leg_move.x =
        wSin(l_ssp_start_time_, x_move_period_time_,
             x_move_phase_shift_ +
                 2 * M_PI / x_move_period_time_ * l_ssp_start_time_,
             x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y =
        wSin(l_ssp_start_time_, y_move_period_time_,
             y_move_phase_shift_ +
                 2 * M_PI / y_move_period_time_ * l_ssp_start_time_,
             y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z =
        wSin(l_ssp_start_time_, z_move_period_time_,
             z_move_phase_shift_ +
                 2 * M_PI / z_move_period_time_ * l_ssp_start_time_,
             z_move_amplitude_, z_move_amplitude_shift_);
    left_leg_move.yaw =
        wSin(l_ssp_start_time_, a_move_period_time_,
             a_move_phase_shift_ +
                 2 * M_PI / a_move_period_time_ * l_ssp_start_time_,
    ...
    right_leg_move.y =
        wSin(l_ssp_start_time_, y_move_period_time_,
             ...
             -y_move_amplitude_, -y_move_amplitude_shift_);
    right_leg_move.z =
        wSin(r_ssp_start_time_, z_move_period_time_,
             z_move_phase_shift_ +
                 2 * M_PI / z_move_period_time_ * r_ssp_start_time_,
             z_move_amplitude_, z_move_amplitude_shift_);
    right_leg_move.yaw =
        wSin(l_ssp_start_time_, a_move_period_time_,
             a_move_phase_shift_ +
    ...
    left_leg_move.x =
        wSin(time_, x_move_period_time_,
             x_move_phase_shift_ +
                 2 * M_PI / x_move_period_time_ * l_ssp_start_time_,
             x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y =
        wSin(time_, y_move_period_time_,
             y_move_phase_shift_ +
                 2 * M_PI / y_move_period_time_ * l_ssp_start_time_,
             y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z =
        wSin(time_, z_move_period_time_,
             z_move_phase_shift_ +
                 2 * M_PI / z_move_period_time_ * l_ssp_start_time_,
             z_move_amplitude_, z_move_amplitude_shift_);
    left_leg_move.yaw =
        wSin(time_, a_move_period_time_,
             a_move_phase_shift_ +
                 2 * M_PI / a_move_period_time_ * l_ssp_start_time_,
             a_move_amplitude_, a_move_amplitude_shift_);
    right_leg_move.x =
    ...
    left_leg_move.x =
        wSin(l_ssp_end_time_, x_move_period_time_,
             x_move_phase_shift_ +
                 2 * M_PI / x_move_period_time_ * l_ssp_start_time_,
             x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y =
        wSin(l_ssp_end_time_, y_move_period_time_,
             y_move_phase_shift_ +
                 2 * M_PI / y_move_period_time_ * l_ssp_start_time_,
             y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z =
        wSin(l_ssp_end_time_, z_move_period_time_,
             z_move_phase_shift_ +
                 2 * M_PI / z_move_period_time_ * l_ssp_start_time_,
             z_move_amplitude_, z_move_amplitude_shift_);
    left_leg_move.yaw =
        wSin(l_ssp_end_time_, a_move_period_time_,
             a_move_phase_shift_ +
                 2 * M_PI / a_move_period_time_ * l_ssp_start_time_,
             a_move_amplitude_, a_move_amplitude_shift_);
    right_leg_move.x =
    ...
    left_leg_move.x =
        wSin(time_, x_move_period_time_,
             x_move_phase_shift_ +
                 2 * M_PI / x_move_period_time_ * r_ssp_start_time_ + M_PI,
             x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y =
        wSin(time_, y_move_period_time_,
             y_move_phase_shift_ +
                 2 * M_PI / y_move_period_time_ * r_ssp_start_time_ + M_PI,
             y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z =
        wSin(l_ssp_end_time_, z_move_period_time_,
             z_move_phase_shift_ +
    ...
    left_leg_move.yaw =
        wSin(time_, a_move_period_time_,
             a_move_phase_shift_ +
                 2 * M_PI / a_move_period_time_ * r_ssp_start_time_ + M_PI,
             a_move_amplitude_, a_move_amplitude_shift_);
    right_leg_move.x =
        wSin(time_, x_move_period_time_,
             x_move_phase_shift_ +
    ...
    left_leg_move.x =
        wSin(r_ssp_end_time_, x_move_period_time_,
             x_move_phase_shift_ +
                 2 * M_PI / x_move_period_time_ * r_ssp_start_time_ + M_PI,
             x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y =
        wSin(r_ssp_end_time_, y_move_period_time_,
             y_move_phase_shift_ +
                 2 * M_PI / y_move_period_time_ * r_ssp_start_time_ + M_PI,
             y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z =
        wSin(l_ssp_end_time_, z_move_period_time_,
             ...
             z_move_amplitude_, z_move_amplitude_shift_);
    left_leg_move.yaw =
        wSin(r_ssp_end_time_, a_move_period_time_,
             a_move_phase_shift_ +
                 2 * M_PI / a_move_period_time_ * r_ssp_start_time_ + M_PI,
             a_move_amplitude_, a_move_amplitude_shift_);
    right_leg_move.x =
    ...
    right_leg_move.y =
        wSin(r_ssp_end_time_, y_move_period_time_,
             ...
             -y_move_amplitude_, -y_move_amplitude_shift_);
    right_leg_move.z =
        wSin(r_ssp_end_time_, z_move_period_time_,
             z_move_phase_shift_ +
                 2 * M_PI / z_move_period_time_ * r_ssp_start_time_,
             z_move_amplitude_, z_move_amplitude_shift_);
    right_leg_move.yaw =
  ...
  double leg_length = humanoid_robot_intelligence_control_system_kd_->thigh_length_m_ + humanoid_robot_intelligence_control_system_kd_->calf_length_m_ +
                      humanoid_robot_intelligence_control_system_kd_->ankle_length_m_;
  ...
  // right leg
  if (humanoid_robot_intelligence_control_system_kd_->calcInverseKinematicsForRightLeg(
          &leg_angle[0], ep[0], ep[1], ep[2], ep[3], ep[4], ep[5]) == false) {
    printf("IK not Solved EPR : %f %f %f %f %f %f\n", ep[0], ep[1], ep[2],
           ep[3], ep[4], ep[5]);
    return false;
  }

  if (humanoid_robot_intelligence_control_system_kd_->calcInverseKinematicsForLeftLeg(
          &leg_angle[6], ep[6], ep[7], ep[8], ep[9], ep[10], ep[11]) == false) {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (i == joint_table_["r_hip_roll"]) // R_HIP_ROLL
      offset += humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_hip_roll") * pelvis_offset_r;
    else if (i == joint_table_["l_hip_roll"]) // L_HIP_ROLL
      offset += humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_hip_roll") * pelvis_offset_l;
    else if (i == joint_table_["r_hip_pitch"])
      offset -= humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_hip_pitch") * hit_pitch_offset_;
    else if (i == joint_table_["l_hip_pitch"]) // R_HIP_PITCH or L_HIP_PITCH
      offset -= humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_hip_pitch") * hit_pitch_offset_;
...
void WalkingModule::computeArmAngle(double *arm_angle) {
  ...
  if (x_move_amplitude_ == 0) {
    ...
    arm_angle[0] = wSin(time_, period_time_, M_PI * 1.5,
                        -x_move_amplitude_ * arm_swing_gain_ * 1000, 0) *
                   humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_sho_pitch") * DEGREE2RADIAN;
    arm_angle[1] = wSin(time_, period_time_, M_PI * 1.5,
                        x_move_amplitude_ * arm_swing_gain_ * 1000, 0) *
                   humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_sho_pitch") * DEGREE2RADIAN;
  }
}

void WalkingModule::sensoryFeedback(const double &rlGyroErr,
  ...
  balance_angle[joint_table_["r_hip_roll"]] =
      humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_hip_roll") * internal_gain * rlGyroErr *
      walking_param_.balance_hip_roll_gain; // R_HIP_ROLL
  balance_angle[joint_table_["l_hip_roll"]] =
      humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_hip_roll") * internal_gain * rlGyroErr *
      walking_param_.balance_hip_roll_gain; // L_HIP_ROLL

  balance_angle[joint_table_["r_knee"]] =
      -humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_knee") * internal_gain * fbGyroErr *
      walking_param_.balance_knee_gain; // R_KNEE
  balance_angle[joint_table_["l_knee"]] =
      -humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_knee") * internal_gain * fbGyroErr *
      walking_param_.balance_knee_gain; // L_KNEE

  balance_angle[joint_table_["r_ank_pitch"]] =
      -humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_ank_pitch") * internal_gain * fbGyroErr *
      walking_param_.balance_ankle_pitch_gain; // R_ANKLE_PITCH
  balance_angle[joint_table_["l_ank_pitch"]] =
      -humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_ank_pitch") * internal_gain * fbGyroErr *
      walking_param_.balance_ankle_pitch_gain; // L_ANKLE_PITCH

  balance_angle[joint_table_["r_ank_roll"]] =
      -humanoid_robot_intelligence_control_system_kd_->getJointDirection("r_ank_roll") * internal_gain * rlGyroErr *
      walking_param_.balance_ankle_roll_gain; // R_ANKLE_ROLL
  balance_angle[joint_table_["l_ank_roll"]] =
      -humanoid_robot_intelligence_control_system_kd_->getJointDirection("l_ank_roll") * internal_gain * rlGyroErr *
...
void WalkingModule::loadWalkingParam(const std::string &path) {
  ...
  walking_param_.init_roll_offset =
      doc["roll_offset"].as<double>() * DEGREE2RADIAN;
  walking_param_.init_pitch_offset =
      doc["pitch_offset"].as<double>() * DEGREE2RADIAN;
  walking_param_.init_yaw_offset =
      doc["yaw_offset"].as<double>() * DEGREE2RADIAN;
  walking_param_.hip_pitch_offset =
      doc["hip_pitch_offset"].as<double>() * DEGREE2RADIAN;
  // time
  walking_param_.period_time =
      doc["period_time"].as<double>() * 0.001; // ms -> s
  ...
  walking_param_.balance_hip_roll_gain =
      doc["balance_hip_roll_gain"].as<double>();
  walking_param_.balance_knee_gain = doc["balance_knee_gain"].as<double>();
  walking_param_.balance_ankle_roll_gain =
      doc["balance_ankle_roll_gain"].as<double>();
  walking_param_.balance_ankle_pitch_gain =
      doc["balance_ankle_pitch_gain"].as<double>();
  ...
  walking_param_.pelvis_offset =
      doc["pelvis_offset"].as<double>() * DEGREE2RADIAN;
...
void WalkingModule::saveWalkingParam(std::string &path) {
  ...
  out_emitter << YAML::Key << "x_offset" << YAML::Value
              << walking_param_.init_x_offset;
  out_emitter << YAML::Key << "y_offset" << YAML::Value
              << walking_param_.init_y_offset;
  out_emitter << YAML::Key << "z_offset" << YAML::Value
              << walking_param_.init_z_offset;
  out_emitter << YAML::Key << "roll_offset" << YAML::Value
              << walking_param_.init_roll_offset * RADIAN2DEGREE;
  out_emitter << YAML::Key << "pitch_offset" << YAML::Value
              << walking_param_.init_pitch_offset * RADIAN2DEGREE;
  out_emitter << YAML::Key << "yaw_offset" << YAML::Value
              << walking_param_.init_yaw_offset * RADIAN2DEGREE;
  out_emitter << YAML::Key << "hip_pitch_offset" << YAML::Value
              << walking_param_.hip_pitch_offset * RADIAN2DEGREE;
  out_emitter << YAML::Key << "period_time" << YAML::Value
              << walking_param_.period_time * 1000;
  out_emitter << YAML::Key << "dsp_ratio" << YAML::Value
              << walking_param_.dsp_ratio;
  out_emitter << YAML::Key << "step_forward_back_ratio" << YAML::Value
              << walking_param_.step_fb_ratio;
  out_emitter << YAML::Key << "foot_height" << YAML::Value
              << walking_param_.z_move_amplitude;
  out_emitter << YAML::Key << "swing_right_left" << YAML::Value
              << walking_param_.y_swap_amplitude;
  out_emitter << YAML::Key << "swing_top_down" << YAML::Value
              << walking_param_.z_swap_amplitude;
  out_emitter << YAML::Key << "pelvis_offset" << YAML::Value
              << walking_param_.pelvis_offset * RADIAN2DEGREE;
  out_emitter << YAML::Key << "arm_swing_gain" << YAML::Value
              << walking_param_.arm_swing_gain;
  out_emitter << YAML::Key << "balance_hip_roll_gain" << YAML::Value
              << walking_param_.balance_hip_roll_gain;
  out_emitter << YAML::Key << "balance_knee_gain" << YAML::Value
              << walking_param_.balance_knee_gain;
  out_emitter << YAML::Key << "balance_ankle_roll_gain" << YAML::Value
              << walking_param_.balance_ankle_roll_gain;
  out_emitter << YAML::Key << "balance_ankle_pitch_gain" << YAML::Value
              << walking_param_.balance_ankle_pitch_gain;
...
void WalkingModule::iniPoseTraGene(double mov_time) {
  ...
  for (int id = 0; id <= result_.size(); id++) {
    ...
    tra = humanoid_robot_intelligence_control_system_framework::calcMinimumJerkTra(ini_value, 0.0, 0.0, tar_value,
  ...
  if (DEBUG)
    std::cout << "Generate Trajecotry : " << mov_time << "s [" << all_time_steps
              << "]" << std::endl;
