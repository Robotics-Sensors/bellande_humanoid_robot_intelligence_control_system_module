...
#include "humanoid_robot_intelligence_control_system_action_module/action_module.h"
#include <sstream>
#include <stdio.h>
...
ActionModule::ActionModule()
    : control_cycle_msec_(8), PRE_SECTION(0), MAIN_SECTION(1), POST_SECTION(2),
      PAUSE_SECTION(3), ZERO_FINISH(0), NONE_ZERO_FINISH(1),
      DEBUG_PRINT(false) {
  /////////////// Const Variable
  /**************************************
   * Section             /----\
...
void ActionModule::initialize(const int control_cycle_msec,
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::Dynamixel *>::iterator it =
    ...
    action_result_[joint_name] = new humanoid_robot_intelligence_control_system_framework::DynamixelState();
  ...
  std::string path =
      ros::package::getPath("humanoid_robot_intelligence_control_system_action_module") + "/data/motion_4095.bin";
  std::string action_file_path =
      ros_node.param<std::string>("action_file_path", path);
...
void ActionModule::queueThread() {
  ...
  ros::Subscriber action_page_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/action/page_num", 0, &ActionModule::pageNumberCallback, this);
  ros::Subscriber start_action_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/action/start_action", 0,
                         &ActionModule::startActionCallback, this);

  /* ROS Service Callback Functions */
  ros::ServiceServer is_running_server =
      ros_node.advertiseService("/humanoid_robot_intelligence_control_system/action/is_running",
...
bool ActionModule::isRunningServiceCallback(
    humanoid_robot_intelligence_control_system_action_module_msgs::IsRunning::Request &req,
...
void ActionModule::pageNumberCallback(const std_msgs::Int32::ConstPtr &msg) {
  ...
  if (msg->data == -1) {
    ...
    for (std::map<std::string, bool>::iterator joints_enable_it =
             action_joints_enable_.begin();
...
void ActionModule::startActionCallback(
    const humanoid_robot_intelligence_control_system_action_module_msgs::StartAction::ConstPtr &msg) {
  if (enable_ == false) {
    std::string status_msg = "Action Module is not enabled";
    ROS_INFO_STREAM(status_msg);
    publishStatusMsg(humanoid_robot_intelligence_control_system_controller_msgs::StatusMsg::STATUS_ERROR,
  ...
  if (msg->page_num == -1) {
    ...
    std::map<std::string, bool>::iterator joints_enable_it =
        action_joints_enable_.begin();
    for (int joint_idx = 0; joint_idx < joint_name_array_size; joint_idx++) {
      joints_enable_it =
          action_joints_enable_.find(msg->joint_name_array[joint_idx]);
...
void ActionModule::process(
  ...
  if (action_module_enabled_ == true) {
    for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::Dynamixel *>::iterator
             dxls_it = dxls.begin();
         dxls_it != dxls.end(); dxls_it++) {
      std::string joint_name = dxls_it->first;

      std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
          result_it = result_.find(joint_name);
      if (result_it == result_.end())
        continue;
      else {
        result_it->second->goal_position_ =
            dxls_it->second->dxl_state_->goal_position_;
        action_result_[joint_name]->goal_position_ =
            dxls_it->second->dxl_state_->goal_position_;
      }
    }
  ...
  for (std::map<std::string, bool>::iterator action_enable_it =
           action_joints_enable_.begin();
       action_enable_it != action_joints_enable_.end(); action_enable_it++) {
    if (action_enable_it->second == true)
      result_[action_enable_it->first]->goal_position_ =
...
void ActionModule::onModuleEnable() { action_module_enabled_ = true; }
...
void ActionModule::stop() { stop_playing_ = true; }

bool ActionModule::isRunning() { return playing_; }
...
bool ActionModule::loadFile(std::string file_name) {
  ...
  if (ftell(action) != (long)(sizeof(action_file_define::Page) *
...
void ActionModule::brake() { playing_ = false; }
...
bool ActionModule::loadPage(int page_number, action_file_define::Page *page) {
  ...
  if (fread(page, 1, sizeof(action_file_define::Page), action_file_) !=
      sizeof(action_file_define::Page))
    return false;
...
bool ActionModule::savePage(int page_number, action_file_define::Page *page) {
  ...
  if (fwrite(page, 1, sizeof(action_file_define::Page), action_file_) !=
      sizeof(action_file_define::Page))
    return false;
...
void ActionModule::resetPage(action_file_define::Page *page) {
  ...
  page->header.schedule =
      action_file_define::TIME_BASE_SCHEDULE; // default time base
...
void ActionModule::enableAllJoints() {
  for (std::map<std::string, bool>::iterator it = action_joints_enable_.begin();
       it != action_joints_enable_.end(); it++) {
    it->second = true;
  }
}
...
void ActionModule::actionPlayProcess(
  ...
  ///////////////// Static Variable
  static uint16_t
      start_angle[action_file_define::MAXNUM_JOINTS]; // Start point of
                                                      // interpolation
  static uint16_t
      target_angle[action_file_define::MAXNUM_JOINTS]; // Target point of
                                                       // interpolation
  static int16_t
      moving_angle[action_file_define::MAXNUM_JOINTS]; // Total Moving Angle
  static int16_t
      main_angle[action_file_define::MAXNUM_JOINTS]; // Moving angle at Constant
                                                     // Velocity Section
  static int16_t
      accel_angle[action_file_define::MAXNUM_JOINTS]; // Moving angle at
                                                      // Acceleration Section
  static int16_t
      main_speed[action_file_define::MAXNUM_JOINTS]; // Target constant velocity
  static int16_t
      last_out_speed[action_file_define::MAXNUM_JOINTS]; // Velocity of Previous
                                                         // State
  static int16_t
      goal_speed[action_file_define::MAXNUM_JOINTS]; // Target velocity
  static uint8_t
      finish_type[action_file_define::MAXNUM_JOINTS]; // Desired State at Target
                                                      // angle
  ...
  if (first_driving_start_ == true) // First start
  {
    ...
    for (unsigned int joint_index = 0;
         joint_index < action_file_define::MAXNUM_JOINTS; joint_index++) {
      id = joint_index;
      std::string joint_name = "";

      std::map<int, std::string>::iterator id_to_name_it =
          joint_id_to_name_.find(id);
      if (id_to_name_it == joint_id_to_name_.end())
        continue;
      else
        joint_name = id_to_name_it->second;

      std::map<std::string, humanoid_robot_intelligence_control_system_framework::Dynamixel *>::iterator dxls_it =
          dxls.find(joint_name);
      if (dxls_it == dxls.end())
        continue;
      else {
        double goal_joint_angle_rad =
            dxls_it->second->dxl_state_->goal_position_;
  ...
  if (unit_time_count < unit_time_num) // Ongoing
  {
    ...
    if (section == PAUSE_SECTION) {
      ...
      for (unsigned int joint_index = 0;
        ...
        std::map<std::string, humanoid_robot_intelligence_control_system_framework::Dynamixel *>::iterator
        ...
        if (dxls_it == dxls.end()) {
          ...
          if (moving_angle[id] == 0) {
            action_result_[joint_name]->goal_position_ =
                convertw4095ToRad(start_angle[id]);
          } else {
            if (section == PRE_SECTION) {
              speed_n = (short)(((long)(main_speed[id] - last_out_speed[id]) *
                                 unit_time_count) /
                                unit_time_num);
              goal_speed[id] = last_out_speed[id] + speed_n;
              accel_angle[id] =
                  (short)((((long)(last_out_speed[id] + (speed_n >> 1)) *
                            unit_time_count * 144) /
                           15) >>
                          9);

              action_result_[joint_name]->goal_position_ =
                  convertw4095ToRad(start_angle[id] + accel_angle[id]);
            } else if (section == MAIN_SECTION) {
              action_result_[joint_name]->goal_position_ = convertw4095ToRad(
                  start_angle[id] +
                  (short int)(((long)(main_angle[id]) * unit_time_count) /
                              unit_time_num));
            ...
            } else // POST_SECTION
            {
              if (unit_time_count == (unit_time_num - 1)) {
                ...
                action_result_[joint_name]->goal_position_ =
                    convertw4095ToRad(target_angle[id]);
              } else {
                if (finish_type[id] == ZERO_FINISH) {
                  speed_n = (short int)(((long)(0 - last_out_speed[id]) *
                                         unit_time_count) /
                                        unit_time_num);
                  goal_speed[id] = last_out_speed[id] + speed_n;
  ...
  } else if (unit_time_count >=
             unit_time_num) // If current section is completed
  {
    unit_time_count = 0;
    ...
    if (section == PRE_SECTION) {
      ...
      for (unsigned int joint_index = 0;
        ...
        if (finish_type[id] == NONE_ZERO_FINISH) {
          if ((unit_time_total_num - accel_step) ==
              0) // if there is not any constant velocity section
            main_angle[id] = 0;
          else
            main_angle[id] =
                (short)((((long)(moving_angle[id] - accel_angle[id])) *
                         unit_time_num) /
                        (unit_time_total_num - accel_step));
        } else
          // ZERO_FINISH
          main_angle[id] =
              moving_angle[id] - accel_angle[id] -
    ...
    if (section == PRE_SECTION) {
      ...
      if (page_step_count_ >
          play_page_.header
              .stepnum) // If motion playing of present page is finished
      {
        // copy next page
      ...
      if (page_step_count_ == play_page_.header.stepnum) // If this is last step
      {
        ...
        if (next_play_page == 0) // If there is no NEXT page, the motion playing
                                 // will be finished after current step.
          playing_finished_ = true;
        else {
          // load next page
          if (play_page_idx_ != next_play_page)
          ...
          // If there is no playing information, the motion playing will be
          // finished after current step.
          if (next_play_page_.header.repeat == 0 ||
              next_play_page_.header.stepnum == 0)
            playing_finished_ = true;
        }
      }
      ...
      //////// Calc Step Parameter
      pause_time =
          (((unsigned short)play_page_.step[page_step_count_ - 1].pause) << 5) /
          play_page_.header.speed;
      max_speed = ((unsigned short)play_page_.step[page_step_count_ - 1].time *
                   (unsigned short)play_page_.header.speed) >>
                  5;
      if (max_speed == 0)
        max_speed = 1;
      max_angle = 0;
      ...
      for (unsigned int joint_index = 0;
        ...
        if (play_page_.step[page_step_count_ - 1].position[id] &
            action_file_define::INVALID_BIT_MASK)
          curr_target_angle = target_angle[id];
        else
          curr_target_angle =
              play_page_.step[page_step_count_ - 1].position[id];
        ...
        if (page_step_count_ ==
            play_page_.header.stepnum) // If current step is last step
        {
          if (playing_finished_ == true) // If it will be finished
            next_target_angle = curr_target_angle;
          else {
            if (next_play_page_.step[0].position[id] &
                action_file_define::INVALID_BIT_MASK)
              next_target_angle = curr_target_angle;
            else
          ...
          if (play_page_.step[page_step_count_].position[id] &
              action_file_define::INVALID_BIT_MASK)
            next_target_angle = curr_target_angle;
          else
        ...
        if (((prev_target_angle < curr_target_angle) &&
             (curr_target_angle < next_target_angle)) ||
            ((prev_target_angle > curr_target_angle) &&
             (curr_target_angle > next_target_angle))) {
          // same direction
          direction_changed = 0;
        } else {
        ...
        if (play_page_.header.schedule ==
            action_file_define::SPEED_BASE_SCHEDULE) {
          // MaxAngle1024 update
          if (moving_angle[id] < 0)
            tmp = -moving_angle[id];
          else
      ...
      // calculation the time. And, the calculated time will be divided
      // by 7.8msec(<<7)- calculate there are how many 7.8msec after unit
      // conversion, calculate angle/velocity, and the following code computes
      // how many units of 7.8s occurs within the specified time unit conversion
      // ---  angle :1024->300deg,  velocity: 256 ->720 wUnitTimeNum =
      // ((wMaxAngle1024*300/1024) /(wMaxSpeed256 * 720/256)) /7.8msec;
      //             = ((128*wMaxAngle1024*300/1024) /(wMaxSpeed256 * 720/256))
      //             ;    (/7.8msec == *128) = (wMaxAngle1024*40) /(wMaxSpeed256
      //             *3);
      ...
      if (unit_time_total_num <= (accel_step << 1)) {
        if (unit_time_total_num == 0) {
          ...
          if (accel_step == 0)
            unit_time_total_num =
                0; // Acceleration and constant velocity steps have to be more
                   // than one in order to move
        }
      }
      ...
      for (unsigned int joint_index = 0;
        ...
        start_speed1024_pre_time_256t =
            (long)last_out_speed[id] *
            pre_section_time_256t; //  *300/1024 * 1024/720 * 256 * 2
        moving_angle_speed1024_scale_256t_2t =
            (((long)moving_angle[id]) * 2560L) / 12;

        if (finish_type[id] == ZERO_FINISH)
          main_speed[id] = (short int)((moving_angle_speed1024_scale_256t_2t -
                                        start_speed1024_pre_time_256t) /
                                       divider2);
        else
          main_speed[id] = (short int)((moving_angle_speed1024_scale_256t_2t -
