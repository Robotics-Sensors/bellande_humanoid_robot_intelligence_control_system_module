...
//           (PortHandler *)PortHandler::getPortHandler(g_device_name.c_str());
...
//       // check dxls torque.
//       uint8_t torque = 0;
//       packet_handler->read1ByteTxRx(port_handler, DEFAULT_DXL_ID,
//                                     TORQUE_ON_CTRL_TABLE, &torque);
...
//            controller->robot_->port_default_device_.begin();
//        map_it != controller->robot_->port_default_device_.end(); map_it++) {
//     std::string default_device_name = map_it->second;
//     controller->read1Byte(default_device_name, TORQUE_ON_CTRL_TABLE,
//                           &torque_result);
...
//   ros::init(argc, argv,
//   "humanoid_robot_intelligence_control_system_manager"); ros::NodeHandle nh;
...
//   ros::Subscriber dxl_torque_sub =
//       nh.subscribe("/humanoid_robot_intelligence_control_system/dxl_torque",
//       1,
//                    dxlTorqueCheckCallback);
//   g_init_pose_pub = nh.advertise<std_msgs::String>(
//       "/humanoid_robot_intelligence_control_system/base/ini_pose", 0);
//   g_demo_command_pub =
//       nh.advertise<std_msgs::String>("/ball_tracker/command", 0);
...
//       int _return = packet_handler->write1ByteTxRx(
//           port_handler, SUB_CONTROLLER_ID, POWER_CTRL_TABLE, 1);

//       if (_return != 0)
//         ROS_ERROR("Torque on DXLs! [%s]",
//                   packet_handler->getRxPacketError(_return));
//       else
...
//     int _return = packet_handler->write2ByteTxRx(
//         port_handler, SUB_CONTROLLER_ID, RGB_LED_CTRL_TABLE, led_value);

//     if (_return != 0)
//       ROS_ERROR("Fail to control LED [%s]",
//                 packet_handler->getRxPacketError(_return));
...
//   controller->addMotionModule((MotionModule
//   *)HeadControlModule::getInstance());
//   controller->addMotionModule((MotionModule *)WalkingModule::getInstance());
//   controller->addMotionModule(
//       (MotionModule *)DirectControlModule::getInstance());
//   controller->addMotionModule(
//       (MotionModule *)OnlineWalkingModule::getInstance());
