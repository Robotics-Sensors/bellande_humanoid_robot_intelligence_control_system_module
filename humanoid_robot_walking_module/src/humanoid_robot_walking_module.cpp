...
void WalkingModule::initialize(const int control_cycle_msec,
  ...
  std::string default_param_path =
      ros::package::getPath("humanoid_robot_walking_module") + "/config/param.yaml";
...
// m, rad
// for default humanoid_robot: it was used from previos version(OP2) but it's not using
// now.
...
bool WalkingModule::computeLegAngle(double *leg_angle) {
  ...
  double leg_length = humanoid_robot_kd_->thigh_length_m_ + humanoid_robot_kd_->calf_length_m_ +
                      humanoid_robot_kd_->ankle_length_m_;
  ...
  // right leg
  if (humanoid_robot_kd_->calcInverseKinematicsForRightLeg(
  ...
  if (humanoid_robot_kd_->calcInverseKinematicsForLeftLeg(
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (i == joint_table_["r_hip_roll"]) // R_HIP_ROLL
      offset += humanoid_robot_kd_->getJointDirection("r_hip_roll") * pelvis_offset_r;
    else if (i == joint_table_["l_hip_roll"]) // L_HIP_ROLL
      offset += humanoid_robot_kd_->getJointDirection("l_hip_roll") * pelvis_offset_l;
    else if (i == joint_table_["r_hip_pitch"])
      offset -= humanoid_robot_kd_->getJointDirection("r_hip_pitch") * hit_pitch_offset_;
    else if (i == joint_table_["l_hip_pitch"]) // R_HIP_PITCH or L_HIP_PITCH
      offset -= humanoid_robot_kd_->getJointDirection("l_hip_pitch") * hit_pitch_offset_;
...
void WalkingModule::computeArmAngle(double *arm_angle) {
  ...
  if (x_move_amplitude_ == 0) {
    ...
    arm_angle[0] = wSin(time_, period_time_, M_PI * 1.5,
                   ...
                   humanoid_robot_kd_->getJointDirection("r_sho_pitch") * DEGREE2RADIAN;
    arm_angle[1] = wSin(time_, period_time_, M_PI * 1.5,
                        x_move_amplitude_ * arm_swing_gain_ * 1000, 0) *
                   humanoid_robot_kd_->getJointDirection("l_sho_pitch") * DEGREE2RADIAN;
  }
}
