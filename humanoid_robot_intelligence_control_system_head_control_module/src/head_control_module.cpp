...
#include "humanoid_robot_intelligence_control_system_head_control_module/head_control_module.h"
#include <stdio.h>
...
HeadControlModule::HeadControlModule()
    : control_cycle_msec_(0), stop_process_(false), is_moving_(false),
      is_direct_control_(true), tra_count_(0), tra_size_(0), moving_time_(3.0),
      scan_state_(NoScan), has_goal_position_(false), angle_unit_(35),
      DEBUG(false) {
  enable_ = false;
  module_name_ = "head_control_module";
  control_mode_ = humanoid_robot_intelligence_control_system_framework::PositionControl;

  result_["head_pan"] = new humanoid_robot_intelligence_control_system_framework::DynamixelState();
  result_["head_tilt"] = new humanoid_robot_intelligence_control_system_framework::DynamixelState();
...
HeadControlModule::~HeadControlModule() { queue_thread_.join(); }
...
void HeadControlModule::initialize(const int control_cycle_msec,
  ...
  queue_thread_ =
      boost::thread(boost::bind(&HeadControlModule::queueThread, this));
...
void HeadControlModule::queueThread() {
  ...
  ros::Subscriber set_head_joint_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/head_control/set_joint_states", 1,
                         &HeadControlModule::setHeadJointCallback, this);
  ros::Subscriber set_head_joint_offset_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/head_control/set_joint_states_offset", 1,
                         &HeadControlModule::setHeadJointOffsetCallback, this);
  ros::Subscriber set_head_scan_sub =
      ros_node.subscribe("/humanoid_robot_intelligence_control_system/head_control/scan_command", 1,
    ...
    if (joint_it != using_joint_name_.end()) {
      ...
      if (is_checked == false) {
        ROS_ERROR_STREAM("Failed to find limit angle \n    id : "
                         << joint_index
                         << ", value : " << (target_position_ * 180 / M_PI));
      }
      ...
      double angle_unit = is_offset ? (angle_unit_ * M_PI / 180 * 1.5)
                                    : (angle_unit_ * M_PI / 180);
...
void HeadControlModule::setHeadScanCallback(
    const std_msgs::String::ConstPtr &msg) {
  if (enable_ == false) {
    ROS_ERROR_THROTTLE(
        1, "Head control module is not enabled, scan command is canceled.");
    return;
  } else
  ...
  if (msg->data == "scan" && scan_state_ == NoScan) {
    std::srand(
        std::time(NULL)); // use current time as seed for random generator
...
bool HeadControlModule::checkAngleLimit(const int joint_index,
...
void HeadControlModule::process(
...
bool HeadControlModule::isRunning() { return is_moving_; }

void HeadControlModule::onModuleEnable() { scan_state_ = NoScan; }
...
void HeadControlModule::generateScanTra(const int head_direction) {
  ...
  case TopLeft: {
    target_position_.coeffRef(0, using_joint_name_["head_pan"]) =
        min_angle_[using_joint_name_["head_pan"]] * 0.6;
    target_position_.coeffRef(0, using_joint_name_["head_tilt"]) =
        min_angle_[using_joint_name_["head_tilt"]] * 0.25;
    break;
  }

  case TopRight: {
    target_position_.coeffRef(0, using_joint_name_["head_pan"]) =
        max_angle_[using_joint_name_["head_pan"]] * 0.6;
    target_position_.coeffRef(0, using_joint_name_["head_tilt"]) =
        min_angle_[using_joint_name_["head_tilt"]] * 0.25;
    break;
  }
  ...
  case BottomLeft: {
    target_position_.coeffRef(0, using_joint_name_["head_pan"]) =
        min_angle_[using_joint_name_["head_pan"]] * 0.45;
    target_position_.coeffRef(0, using_joint_name_["head_tilt"]) =
        min_angle_[using_joint_name_["head_tilt"]] * 0.8;
    break;
  }

  case BottomRight: {
    target_position_.coeffRef(0, using_joint_name_["head_pan"]) =
        max_angle_[using_joint_name_["head_pan"]] * 0.45;
    target_position_.coeffRef(0, using_joint_name_["head_tilt"]) =
        min_angle_[using_joint_name_["head_tilt"]] * 0.8;
    break;
  }
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    // set time
    double calc_moving_time = fabs(goal_position_.coeff(0, index) -
                                   target_position_.coeff(0, index)) /
                              (60.0 * M_PI / 180.0);
    if (calc_moving_time > moving_time_)
      moving_time_ = calc_moving_time;
  }
...
void HeadControlModule::jointTraGeneThread() {
  ...
  for (std::map<std::string, humanoid_robot_intelligence_control_system_framework::DynamixelState *>::iterator
    ...
    Eigen::MatrixXd tra =
        calcMinimumJerkTraPVA(ini_value, ini_vel, ini_accel, tar_value, 0.0,
                              0.0, smp_time, moving_time_);

    calc_joint_tra_.block(0, index, all_time_steps, 1) =
        tra.block(0, 0, all_time_steps, 1);
    calc_joint_vel_tra_.block(0, index, all_time_steps, 1) =
        tra.block(0, 1, all_time_steps, 1);
    calc_joint_accel_tra_.block(0, index, all_time_steps, 1) =
        tra.block(0, 2, all_time_steps, 1);
  }
